...
class Form_Field extends Base {
	 ...
	 */
	public function add_attribute($attribute_name, $attribute_values){
	...
	/**
	 * Create a new controller and get view: In this way variables inside "$this->view[]" can't be overwritten
	 */
	public function get_view($optional_params = array()){
		$controller		= new Controller($this->context);

		return $this->get_controller_view($controller, $optional_params);
	}

	public function get_default_controller_view_params($optional_params = array()){
		$params		= array(
	...
	public function get_controller_view($controller, $optional_params = array()){
		...
		$view		= $controller->get_framework_view(
							$this->get_framework_view_module_name(), 
							$this->get_framework_view_path(), 
							$this->get_custom_field_view_params($params)
						);
	...
	public function get_custom_view($view){
		return $view;
	}
	 ...
	 */
	public function print_field_label_html(){
		echo $this->get_field_label_html();
	}

	public function get_field_label_html(){

		$html		= "";

		if(!empty($this->get_label()) && $this->get_show_label() == true){
			$html	= "<label class=\"yps-field-label\" for=\"\">";
		
			if($this->get_allow_empty() == false){
				$html .= "<span class='yps-mandatory-field'>*</span> ";
			}
